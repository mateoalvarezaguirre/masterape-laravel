...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ config('app.name', 'Masterape Bookstore') }}</title>

        <link rel="shortcut icon" href="{{ asset('images/logo_light.svg') }}" type="image/x-icon">
    ...
    <body x-cloak x-data="{darkMode: $persist(false)}" :class="{'dark': darkMode === true }" class="font-sans antialiased">
        <div class="min-h-screen bg-custom-light dark:bg-neutral-900">
            @include('layouts.navigation')

            <!-- Page Heading -->
            @isset($header)
                <header class="bg-custom-light dark:bg-neutral-900 shadow">
